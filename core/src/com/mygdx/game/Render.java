...
import com.mygdx.game.Objects.TextObject;
...
public class Render {
    ...
    public Render()
    {
        ...
        backGround = new Texture("fons/" + Back.values()[new Random().nextInt(Back.values().length)] + ".png");
        ...
        txPersons.put("Magician",         new TextureRegion(tiles[1][6]));
        ...
        activePerson.put("Magician",      new Animation<>(0.2f, tiles[1][6], tiles[2][6]));
        ...
        actionAnim.put("ExpObject", new Animation<>((float) 1/ MainGame.getFps(), expl));
        ...
        actionAnim.put("ResObject", new Animation<>((float) 1/ MainGame.getFps(), expl));
        ...
        actionAnim.put("ShootObject", new Animation<>((float) 1/ MainGame.getFps(), expl));
    }

    public void render(float delta, ArrayList<TeamPerson> team, ArrayList<BaseObject> actionObjects)
    {
        gameTime += delta;
...
//        font.draw(batch, "FPS: " + Gdx.graphics.getFramesPerSecond(), 8, 20);
    ...
    private void renderAction(ArrayList<BaseObject> actionObjects)
    {
        if (!actionObjects.isEmpty())
        {
            for (BaseObject p : actionObjects)
            {
                if (p instanceof TextObject)
                {
                    font.draw(batch, ((TextObject) p).getText(), p.getPosX(), p.getPosY());
                } else {
                    int x = p.getPosX();
                    int y = p.getPosY();
                    int w = MainGame.getMapTileWidth();
                    int h = MainGame.getMapTileHeight();
    ...
    {
        batch.draw(backGround,1, 0, Gdx.graphics.getWidth(),Gdx.graphics.getHeight());
    }
    ...
    private void drawPerson(TextureRegion frame, Person person, boolean flip)
    {
        int tileWidth = MainGame.getMapTileWidth();
        int tileHeight = MainGame.getMapTileHeight();
